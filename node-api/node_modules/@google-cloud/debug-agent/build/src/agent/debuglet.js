...
const oneplatform_controller_1 = require("./oneplatform-controller");
...
class Debuglet extends events_1.EventEmitter {
    ...
    async start() {
        ...
        if (this.config.useFirebase) {
            ...
            try {
                ...
                this.logger.error('The project ID could not be determined: ' + err.message);
                this.emit('initError', err);
                return;
            }
            this.controller = new oneplatform_controller_1.OnePlatformController(this.debug, this.config, this.logger);
        }
        if (onGCP &&
        ...
        this.debuggee = Debuglet.createDebuggee(project, id, this.config.serviceContext, sourceContext, onGCP, this.debug.packageInfo, platform, this.config.description, 
    ...
    completeBreakpoint_(breakpoint, deleteFromV8 = true) {
        ...
        this.debuggee, breakpoint, (err /*, body*/) => {
            ...
            if (!this.config.useFirebase) {
                // TODO: Address the case when `breakpoint.id` is `undefined`.
                this.completedBreakpointMap[breakpoint.id] = true;
